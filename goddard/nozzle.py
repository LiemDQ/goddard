...
class Nozzle(ABC):
    ...
    def __init__(self, inlet_gas: ct.Solution, exit_conditions: ExitConditions, gamma_s: float | None = None) -> None:
        ...
        self.throat = self.get_throat_conditions(copy_ct_solution(inlet_gas))
        self.sonic_velocity = get_speed_of_sound(self.throat, self.inlet_gamma)
    ...
    def get_exit_conditions(self) -> ct.Solution:
        if self.exit_conditions.supersonic_ratio:
            return self._get_exit_conditions_supr()
        elif self.exit_conditions.pressure_ratio:
    ...
    @abstractmethod    
    def get_throat_conditions(self) -> ct.Solution:
        pass
    ...
    @abstractmethod
    def _get_exit_conditions_supr(self) -> ct.Solution:
        pass
    ...
    def update(self):
        self.throat = self.get_throat_conditions(copy_ct_solution(self.inlet))
        self.sonic_velocity = get_speed_of_sound(self.throat, self.inlet_gamma)
...
class EquilibriumNozzle(Nozzle):
    ...
    def get_throat_conditions(self) -> ct.Solution:
        ...
        - Equilibrium flow
        '''
        gas = copy_ct_solution(self.inlet)
        ...
        while residual > tolerance_throat:
            ...
            sonic_velocity = get_speed_of_sound(gas, gamma_s)
    ...
    def _get_exit_conditions_supr(self) -> ct.Solution:
        ...
        pressure_ratio = np.exp(gamma_s + 1.4 * np.log(self.exit_conditions.supersonic_ratio)) #initial guess 
        ...
        while np.abs(residual) > tolerance:
            ...
            residual = dlogp_dlogA * (np.log(self.exit_conditions.supersonic_ratio) - np.log(Ae_At))
...
class FrozenNozzle(Nozzle):
    ...
    def _get_exit_conditions_supr(self) -> ct.Solution:
        ...
        gas_exit = copy_ct_solution(self.throat)
    ...
    def _get_exit_conditions_pr(self) -> ct.Solution:
        ...
        T_e = self.inlet.T #initial guess
        gas_exit = copy_ct_solution(self.throat)
        gas_exit.P = self.inlet_pressure / self.exit_conditions.pressure_ratio

        dlogV_dlogT_P, dlogV_dlogP_T, cp, gamma_s = get_thermo_properties(gas_exit)
        dlnT = (self.inlet.s - gas_exit.s)/cp
...
def get_exit_conditions_frozen(gas_throat: ct.Mixture, area_ratio, p_chamber, gas_chamber: ct.Solution, gamma_s, velocity):
    ...
    gas_exit = copy_ct_solution(gas_throat)
