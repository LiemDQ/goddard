...
// Rao design: used whenever MocOptions::mode is not DESIGN_MIN_LENGTH, which uses
// DirectMarch (moc_direct_march.cpp) instead.
#include <cmath>
#include <algorithm>
#include <format>
#include <limits>
...
std::vector<CharacteristicPoint> InverseMarch::initial_front(const StartLine& start) {
    if (!start.family.has_value()) {
        // Kliegel-Levine topology (or a test-injected override, seeded through the same
        // path -- see MocNozzle::m_inverse_front_override): already spans axis to wall.
        return start.points;
    }

    // Centered-fan topology. start.points is the marched C+ from the fan's first axis point
    ...
    const NozzleProfile& wall = m_ctx.wall;
    const std::vector<CharacteristicPoint>& data_line = start.points;
    const size_t n_rays = data_line.size();
    if (n_rays < 2 || start.theta_schedule.size() < n_rays) {
    ...
    auto ray_invariants = [&](size_t i, double f, double& K_minus, double& K_plus) {
        const double theta_lip = start.theta_schedule[i];
    ...
    const size_t n_points = std::max<size_t>(static_cast<size_t>(m_ctx.options.num_characteristics), 3);
    ...
    for (size_t j = 0; j < n_points; j++) {
        ...
        MocErrorCode thermo_error = m_ctx.thermo.set_state_from_nu(pt, 0.5 * (K_minus - K_plus), 1.05);
    ...
    {
        MocErrorCode thermo_error = m_ctx.thermo.set_state_from_nu(wall_pt, wall_pt.theta - below.K_plus, below.mach);
...
void InverseMarch::seed(const std::vector<CharacteristicPoint>& front0) {
    if (front0.size() < 2) {
        throw ConvergenceError("Inverse march: initial front has fewer than two points.");
    }
    m_net.add_front(front0);
}

std::optional<InverseMarch::Foot> InverseMarch::trace_foot(
    double x0, double y0, double angle, const std::vector<size_t>& front,
    bool allow_axis_mirror, MocErrorCode& err) const
{
    std::optional<FootHit> hit = allow_axis_mirror
        ? trace_with_axis_mirror(x0, y0, angle, m_net, front)
        : trace_plain(x0, y0, angle, m_net, front);
    if (!hit.has_value()) return std::nullopt;

    const double y_for_fit = hit->mirrored ? -hit->y : hit->y;
    FrontFit fit = lagrange_front_fit(m_net, front, hit->seg, y_for_fit);
    CharacteristicPoint out{};
    out.x = hit->x;
    out.y = hit->y;
    out.theta = hit->mirrored ? -fit.theta : fit.theta;
    err = m_ctx.thermo.set_state_from_nu(out, fit.nu, fit.mach);
    return Foot{out, hit->mirrored};
}

PointResult InverseMarch::solve_interior_point(double x, double y, const std::vector<size_t>& front) {
    ...
    MocErrorCode err = m_ctx.thermo.set_state_from_nu(p4, seed_fit.nu, seed_fit.mach);
    if (err != MocErrorCode::NONE) return {p4, err};

    const bool axisymmetric = (m_ctx.options.flow_type == MocFlowKind::AXISYMMETRIC);
    ...
    for (int iter = 0; iter < max_iters; iter++) {
        MocErrorCode err_m = MocErrorCode::NONE;
        MocErrorCode err_p = MocErrorCode::NONE;
        std::optional<Foot> foot_minus = trace_foot(p4.x, p4.y, angle_minus, front, false, err_m);
        std::optional<Foot> foot_plus = trace_foot(p4.x, p4.y, angle_plus, front, true, err_p);
        if (!foot_minus.has_value() || !foot_plus.has_value()) {
        ...
        if (axisymmetric) {
            const double mu_m = 0.5 * (foot_minus->state.mu + p4.mu);
            const double th_m = 0.5 * (foot_minus->state.theta + p4.theta);
            const double y_m = 0.5 * (foot_minus->state.y + p4.y);
            const double ang_m = 0.5 * (angle_minus + (foot_minus->state.theta - foot_minus->state.mu));
            if (std::abs(y_m) > 1e-12) source_minus = axisymmetric_source(mu_m, th_m, y_m, ang_m);

            const double mu_p = 0.5 * (foot_plus->state.mu + p4.mu);
            const double ang_p = 0.5 * (angle_plus + (foot_plus->state.theta + foot_plus->state.mu));
            if (foot_plus->mirrored) {
                ...
                const double th_p = 0.5 * (foot_plus->state.theta + p4.theta);
                const double y_p = 0.5 * (foot_plus->state.y + p4.y);
                if (std::abs(y_p) > 1e-12) source_plus = axisymmetric_source(mu_p, th_p, y_p, ang_p);
            }
        }

        const double K_minus = (foot_minus->state.theta + foot_minus->state.nu)
            + source_minus * (p4.x - foot_minus->state.x);
        const double K_plus = (foot_plus->state.theta - foot_plus->state.nu)
            - source_plus * (p4.x - foot_plus->state.x);

        const double theta_prev = p4.theta;
        const double nu_prev = p4.nu;
        p4.theta = 0.5 * (K_minus + K_plus);
        err = m_ctx.thermo.set_state_from_nu(p4, 0.5 * (K_minus - K_plus), p4.mach);
        if (err != MocErrorCode::NONE) return {p4, err};
    ...
    MocErrorCode validity = check_point_validity(p4, m_ctx.options.solver_options.abstol, /*require_nonnegative_theta=*/false);
    if (validity != MocErrorCode::NONE) return {p4, validity};
    return {p4, MocErrorCode::NONE};
}

PointResult InverseMarch::solve_axis_point(double x, const std::vector<size_t>& front) {
    CharacteristicPoint axis_point{};
    axis_point.x = x;
    axis_point.y = 0.0;
    axis_point.theta = 0.0;

    const CharacteristicPoint& seed_src = m_net.points[front.front()];
    MocErrorCode err = m_ctx.thermo.set_state_from_nu(axis_point, seed_src.nu, seed_src.mach);
    ...
    constexpr int max_iters = 4;
    for (int iter = 0; iter < max_iters; iter++) {
        MocErrorCode err_foot = MocErrorCode::NONE;
        std::optional<Foot> foot = trace_foot(axis_point.x, 0.0, angle_minus, front, false, err_foot);
        if (!foot.has_value()) return {axis_point, MocErrorCode::NON_DOWNSTREAM_POINT};
        if (err_foot != MocErrorCode::NONE) return {axis_point, err_foot};
        ...
        if (axisymmetric) {
            ...
            const double source = axis_source_correction(foot->state, axis_point);
            nu_new = (foot->state.theta + foot->state.nu) + source;
        } else {
            nu_new = foot->state.theta + foot->state.nu; // planar: K- exactly preserved, no source
        }

        const double nu_prev = axis_point.nu;
        err = m_ctx.thermo.set_state_from_nu(axis_point, nu_new, axis_point.mach);
        if (err != MocErrorCode::NONE) return {axis_point, err};

        const bool converged = std::abs(axis_point.nu - nu_prev) < 1e-9;
        angle_minus = average_cminus_angle(axis_point, foot->state);
        if (converged) break;
    }
    ...
    MocErrorCode validity = check_point_validity(axis_point, m_ctx.options.solver_options.abstol, false);
    if (validity != MocErrorCode::NONE) return {axis_point, validity};
    return {axis_point, MocErrorCode::NONE};
}

PointResult InverseMarch::solve_wall_point(double x, double y, const std::vector<size_t>& front) {
    CharacteristicPoint wall_point{};
    wall_point.x = x;
    wall_point.y = y;
    wall_point.theta = m_ctx.wall.theta_at_interpolated(x); // fixed by the contour

    const CharacteristicPoint& seed_src = m_net.points[front.back()];
    MocErrorCode err = m_ctx.thermo.set_state_from_nu(wall_point, seed_src.nu, seed_src.mach);
    ...
    for (int iter = 0; iter < max_iters; iter++) {
        MocErrorCode err_foot = MocErrorCode::NONE;
        std::optional<Foot> foot = trace_foot(wall_point.x, wall_point.y, angle_plus, front, true, err_foot);
        ...
        err = m_ctx.thermo.set_state_from_nu(wall_point, nu_new, wall_point.mach);
        if (err != MocErrorCode::NONE) return {wall_point, err};

        const bool converged = std::abs(wall_point.nu - nu_prev) < 1e-9;
        angle_plus = average_cplus_angle(foot->state, wall_point);
        if (converged) break;
    }

    wall_point.update_Ks();

    MocErrorCode validity = check_point_validity(wall_point, m_ctx.options.solver_options.abstol, false);
    if (validity != MocErrorCode::NONE) return {wall_point, validity};
    return {wall_point, MocErrorCode::NONE};
}

std::optional<MocFailure> InverseMarch::check_front_spacelike(const std::vector<size_t>& front) const {
    ...
    for (size_t j = 0; j + 1 < M; j++) {
        const CharacteristicPoint& a = m_net.points[front[j]];
        const CharacteristicPoint& b = m_net.points[front[j + 1]];
...
InverseMarch::Step InverseMarch::choose_step(
    const std::vector<size_t>& front, const std::vector<double>& offset,
    double max_abs_offset, int pass) const
{
    (void)max_abs_offset; // used by prescribe_front, not by the step-length computation itself
    const size_t M = front.size();
    ...
    for (size_t j = 0; j < M; j++) {
        const CharacteristicPoint& p = m_net.points[front[j]];
        slope_max = std::max({slope_max, std::abs(std::tan(p.theta + p.mu)),
                              std::abs(std::tan(p.mu - p.theta))});
        if (j + 1 < M) dy_min = std::min(dy_min, m_net.points[front[j + 1]].y - p.y);
    }
    const double dx_cfl = (slope_max > 1e-12)
        ? m_ctx.options.inverse_cfl * dy_min / slope_max
    ...
    if (M >= 3 && m_net.points[front[M - 2]].mu - m_net.points[front[M - 2]].theta > 1e-9) {
        const CharacteristicPoint& top = m_net.points[front[M - 2]];
        const double dy_top = wall_old.y - top.y;
        const double cot_minus = 1.0 / std::tan(top.mu - top.theta);
        const double keep = (offset[M - 2] < 0.0) ? m_ctx.options.front_tilt_decay : 1.0;
    ...
    double dx_turn = std::numeric_limits<double>::max();
    {
        ...
        for (size_t k = 0; k < wall_profile.size(); k++) {
            ...
            if (std::abs(wall_profile.theta_at_interpolated(x_v) - theta_here) > m_ctx.options.max_wall_turn_per_step) {
    ...
    m_ctx.log.debug("STEP pass={} dx_cfl={:.6f} dx_foot={:.6f} dx_turn={:.6f} dx_exit={:.6f} dx={:.6f}",
        pass, dx_cfl, dx_foot, dx_turn, dx_exit, dx);

    return Step{dx, limiter};
}

void InverseMarch::prescribe_front(
    const std::vector<size_t>& front, const std::vector<double>& s,
    ...
    std::vector<double>& x_new, std::vector<double>& y_new) const
{
    ...
    double lambda = 1.0 - m_ctx.options.front_tilt_decay;
...
MocPassDiagnostics InverseMarch::measure_front(
    const std::vector<CharacteristicPoint>& front, int pass, Step step)
{
    ...
    diag.pass = pass;
    diag.front_points = M;
    diag.step_dx = step.dx;
    diag.step_limiter = step.limiter;
    diag.front_axis_x = front.front().x;
    diag.front_wall_x = front.back().x;

    double mn = std::numeric_limits<double>::max();
    double mx = 0.0, total = 0.0;
    for (size_t j = 0; j + 1 < M; j++) {
        const double seg_dx = front[j + 1].x - front[j].x;
        const double seg_dy = front[j + 1].y - front[j].y;
    ...
    diag.front_axis_spacing = std::hypot(front[1].x - front[0].x, front[1].y - front[0].y);
    diag.front_wall_spacing = std::hypot(front[M - 1].x - front[M - 2].x, front[M - 1].y - front[M - 2].y);
    return diag;
}

std::optional<MocFailure> InverseMarch::run() {
    if (m_net.fronts.empty() || m_net.fronts.back().size() < 2) {
        return MocFailure{MocErrorCode::INITIALIZATION_FAILED,
            "Inverse march: initial front has fewer than two points.", 0.0, 0.0, -1};
    }

    std::vector<size_t> front = m_net.fronts.back();
    const size_t M = front.size();

    // Normalized y-distribution of the initial front, reused for every subsequent front.
    std::vector<double> s(M);
    {
        const double y_wall0 = m_net.points[front.back()].y;
        for (size_t j = 0; j < M; j++) {
            s[j] = (y_wall0 > 0.0)
                ? m_net.points[front[j]].y / y_wall0
                : static_cast<double>(j) / static_cast<double>(M - 1);
        }
    }

    if (std::optional<MocFailure> spacelike_failure = check_front_spacelike(front)) {
        return spacelike_failure;
    }
    ...
    for (int pass = 0; pass < max_passes; pass++) {
        ...
        for (size_t j = 0; j < M; j++) {
            offset[j] = m_net.points[front[j]].x - wall_old.x;
            max_abs_offset = std::max(max_abs_offset, std::abs(offset[j]));
        }

        // --- Step 1: step length ---
        Step step = choose_step(front, offset, max_abs_offset, pass);

        if (!(step.dx > 0.0) || !std::isfinite(step.dx)) {
            return MocFailure{MocErrorCode::NON_DOWNSTREAM_POINT,
                std::format("Inverse march: non-positive step length ({:.3e}, limiter {}) at pass {}.",
                    step.dx, to_string(step.limiter), pass),
                wall_old.x, wall_old.y, pass};
        }

        // --- Step 2: new front geometry ---
        std::vector<double> x_new, y_new;
        prescribe_front(front, s, offset, max_abs_offset, step.dx, x_new, y_new);

        m_ctx.log.debug("FRONT pass={} dx={:.6f} limiter={} x_axis={:.6f} x_wall={:.6f}",
            pass, step.dx, to_string(step.limiter), x_new[0], x_new[M - 1]);

        // --- Steps 3-6: the unit processes ---
        std::vector<CharacteristicPoint> new_front_points(M);
        {
            PointResult axis_result = solve_axis_point(x_new[0], front);
        ...
        for (size_t j = 1; j + 1 < M; j++) {
            PointResult r = solve_interior_point(x_new[j], y_new[j], front);
        ...
        {
            PointResult wall_result = solve_wall_point(x_new[M - 1], y_new[M - 1], front);
        ...
        // --- Step 7: bookkeeping ---
        std::vector<size_t> new_front = m_net.add_front(new_front_points);
        pass_diagnostics.push_back(measure_front(new_front_points, pass, step));

        front = new_front;

        if (step.limiter == MocStepLimiter::EXIT) {
