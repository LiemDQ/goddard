...
) : fuel_sln(fuel), oxidizer_sln(oxidizer), product_sln(products)
{}
...
ThermoArray Combustor::solve(const Eigen::ArrayXd& temperatures, const Eigen::ArrayXd& pressures, const MixtureRatios& mr, const CombustionOptions& options) {
    auto mole_fracs = generate_mole_fraction_matrix(mr);

    //NOTE: this assumes the species layout in "products" is the same as in the feed object
    ThermoArray combustion_states = ThermoArray(product_sln, {temperatures.size(), pressures.size(), mr.molar_ratio().size() });
 ...
 */
Eigen::ArrayXXd Combustor::generate_mole_fraction_matrix(const MixtureRatios& mr) {
    
    auto oxidizer_thermo = oxidizer_sln->thermo();
    auto fuel_thermo = fuel_sln->thermo();
    ...
    auto mole_frac_matrix = Eigen::ArrayXXd(mole_ratios.size(), product_sln->thermo()->nSpecies()); 
...
void Combustor::assign_mole_frac_row_entries(
    ...
    auto product_thermo = product_sln->thermo();
